...
def htmlAlanList(label,tip,veri,meta):
    ...
    if("Combobox" in tip):
        if("@code" in meta or "@Code" in meta):
            liste.write("  <td *ngIf=\"dto.parametrelistesiAdi; else void\">{{parametrelistesiAdi[dto."+veri+"].value}}</td>")
        else:
           liste.write("<td *ngIf=\"dto."+veri+"\"><i style=\"margin-left: 30%;\" class=\"material-icons\">done</i></td>\n")
    elif("Text" in tip):
