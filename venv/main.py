...
parser.add_argument('--filterexcel' ,type=str,
                   help ="Liste formunu excele kopyala yolunu ver")
...
def backController():
    ...
    for i in range(1, sheet.nrows):
        ...
        filtre.write("filter.get" +myLib.filterType(str(type).lower(),str(meta).lower())+"(\""+myLib.camelCase(str(veri).split(".").pop()))
        filtre.write("\"" +"),")
        filtre.write("\n");
    filtre.close()
...
def htmlList():
    ...
    for i in range(1,sheet.nrows):
        ...
        myLib.repWords = (label,myLib.camelCase(str(veri).split('.').pop()),myLib.className(str(veri).split('.').pop()),str(meta).split('.').pop(0),args.serverpath,comboParameter)
...
def htmlAlanListesi():
    ...
    for i in range(1,sheet.nrows):
        ...
        tipi = sheet.cell_value(i,2)
        meta = sheet.cell_value(i,3)
        myLib.htmlAlanList(label,tipi,myLib.camelCase(str(veri).split('.').pop()),meta)
...
def htmlEdit():
    ...
    for i in range(1, sheet.nrows):
        ...
        if("code" in meta):
            comboParameter = str(meta).split(':').pop(0)
            comboParameter = comboParameter[0].upper()+comboParameter[1:]
        myLib.checkWords = ("labelName", "tableName1", "className", "camelTable", "objectName","tableIndex","parametre")
        myLib.repWords = (label, str(meta).split('.').pop(0), classVeri, myLib.camelCase(args.tabloAdi), camelVeri,str(meta).split('.').pop(),comboParameter)
...
def htmlBirlestir():
    ...
    myLib.repWords = (str(htmlTemplateFilter.read()), myLib.className(args.tabloAdi), myLib.camelCase(args.tabloAdi),str(htmlAlanLabel.read()),str(htmlAlanList.read()),args.serverpath)
    ...
    myLib.repWords = (str(htmlTemplateEdit.read()), myLib.className(args.tabloAdi), myLib.camelCase(args.tabloAdi),args.serverpath)
...
def dao():
    temp = open("daoEklenecek.txt","a+",encoding="utf8")
    wb = xlrd.open_workbook(args.filterexcel)
    sheet = wb.sheet_by_index(0)
    sheet.cell_value(0, 0)
    for i in range(1, sheet.nrows):
        ...
        meta = sheet.cell_value(i, 4)
        # temp.write("@Param(\"" +)
    # @Param("gorevUnvanId") Long gorevUnvanId
...
def save():
    ...
    os.rename(os.path.join(angulardir,"componentEditReady.txt"),os.path.join(angulardir,str(args.tabloAdi).lower().replace("_","-")+"-edit.component.ts" ))
    os.rename(os.path.join(angulardir, "componentListReady.txt"), os.path.join(angulardir, str(args.tabloAdi).lower().replace("_","-")+"-list.component.ts"))
    os.rename(os.path.join(angulardir, "frontModelReady.txt"),os.path.join(angulardir, myLib.className(str(args.tabloAdi))+ ".ts"))
    os.rename(os.path.join(angulardir, "frontService.txt"),os.path.join(angulardir, myLib.className(str(args.tabloAdi))+ "Service.ts"))
    os.rename(os.path.join(angulardir, "htmlEditReady.txt"),os.path.join(angulardir, str(args.tabloAdi).lower().replace("_","-")+ "-edit.component.html"))
    os.rename(os.path.join(angulardir, "htmlListReady.txt"),os.path.join(angulardir, str(args.tabloAdi).lower().replace("_","-")+ "-list.component.html"))
    os.rename(os.path.join(javadir, "BackController.txt"),os.path.join(javadir, myLib.className(str(args.tabloAdi))+ "Controller.java"))
    os.rename(os.path.join(javadir,"entityReady.txt"),os.path.join(javadir,myLib.className(args.tabloAdi)+".java"))
    os.rename(os.path.join(javadir,"daoEklenecek.txt"),os.path.join(javadir,myLib.className(args.tabloAdi)+"Dao.java"))
    os.rename(os.path.join(javadir, "changelogReady.txt"), os.path.join(javadir, myLib.className(args.tabloAdi) + "changelog.xml"))
